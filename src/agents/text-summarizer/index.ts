...
const AGENT_NAME = 'text-summarizer'
const logger = createAgentLogger(AGENT_NAME)

export async function createTextSummarizer() {
  logger.info('初始化开始')
  ...
  agent.onTask('text_summarization', async (ctx) => {
    ...
    const prompt = loadPrompt(import.meta.url, 'summary.md', { input: inputStr })
