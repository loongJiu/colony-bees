...
const AGENT_NAME = 'brainstormer'
const logger = createAgentLogger(AGENT_NAME)

export async function createBrainstormer() {
  logger.info('初始化开始')
  ...
  agent.onTask('brainstorming', async (ctx) => {
    ...
    const prompt = loadPrompt(import.meta.url, 'brainstorm.md', { input: inputStr })
