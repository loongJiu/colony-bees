...
const AGENT_NAME = 'keyword-extractor'
const logger = createAgentLogger(AGENT_NAME)

export async function createKeywordExtractor() {
  logger.info('初始化开始')
  ...
  agent.onTask('keyword_extraction', async (ctx) => {
    ...
    const prompt = loadPrompt(import.meta.url, 'extract.md', { input: inputStr })
