...
const AGENT_NAME = 'quiz-generator'
const logger = createAgentLogger(AGENT_NAME)

export async function createQuizGenerator() {
  logger.info('初始化开始')
  ...
  agent.onTask('quiz_generation', async (ctx) => {
    ...
    const prompt = loadPrompt(import.meta.url, 'quiz.md', { input: inputStr })
