...
const AGENT_NAME = 'comparison-analyzer'
const logger = createAgentLogger(AGENT_NAME)

export async function createComparisonAnalyzer() {
  logger.info('初始化开始')
  ...
  agent.onTask('comparison_analysis', async (ctx) => {
    ...
    const prompt = loadPrompt(import.meta.url, 'compare.md', { input: inputStr })
