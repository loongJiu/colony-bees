...
const AGENT_NAME = 'task-decomposer'
const logger = createAgentLogger(AGENT_NAME)

export async function createTaskDecomposer() {
  logger.info('初始化开始')
  ...
  agent.onTask('task_decomposition', async (ctx) => {
    ...
    const prompt = loadPrompt(import.meta.url, 'decompose.md', { input: inputStr })
