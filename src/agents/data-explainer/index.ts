...
const AGENT_NAME = 'data-explainer'
const logger = createAgentLogger(AGENT_NAME)

export async function createDataExplainer() {
  logger.info('初始化开始')
  ...
  agent.onTask('data_explanation', async (ctx) => {
    ...
    const prompt = loadPrompt(import.meta.url, 'explain.md', { input: inputStr })
